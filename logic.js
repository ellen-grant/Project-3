document.addEventListener('DOMContentLoaded', () => {
  ...
  d3.json("https://data.cdc.gov/api/views/hfr9-rurv/rows.json").then(function(data) {
    ...
    data.data.forEach((row, index) => {
      // Ensure that row has sufficient columns
      if (row.length <= 22) {
        console.warn(`Row ${index} does not have the expected number of columns:`, row);
        return;
      }

      // Extract the coordinates from the 'Geolocation' field in the response data
      let geolocation = row[22]; 
      
      if (geolocation) {
        // Extract latitude and longitude from 'POINT (longitude latitude)'
        let matches = geolocation.match(/POINT \((-?\d+\.\d+) (-?\d+\.\d+)\)/);
        
        if (matches) {
          let lon = parseFloat(matches[1]); // Longitude is the first capture group
          let lat = parseFloat(matches[2]); // Latitude is the second capture group

          console.log(`Extracted Coordinates - Latitude: ${lat}, Longitude: ${lon}`); // Debugging line

          // Check if coordinates are valid
          if (!isNaN(lat) && !isNaN(lon)) {
            // Create a marker and add it to the marker cluster group
            let marker = L.marker([lat, lon])
              .bindPopup(`<strong>LocationAbbr:</strong> ${row[3]}<br>
                          <strong>Class:</strong> ${row[6]}<br>
                          <strong>StratificationID1:</strong> ${row[28]}<br>
                          <strong>Stratification2:</strong> ${row[21]}<br>
                          <strong>Data_Value:</strong> ${row[12]}`);

            markers.addLayer(marker);
          }
        } else {
          console.warn(`Geolocation does not match expected format for row ${index}: ${geolocation}`);
        }
      } else {
        console.warn(`No geolocation data for row ${index}`);
      }
    });
